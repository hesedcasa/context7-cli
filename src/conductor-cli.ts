#!/usr/bin/env node
/**
 * Conductor CLI - Entry point for the web orchestrator
 */

import { ConductorServer } from './conductor/index.js';
...
async function main() {
	...
	try {
		...
		const server = new ConductorServer(port, repoPath, apiKey);
		...
		console.error('Failed to start Conductor:', error.message);
		process.exit(1);
	}
}
...
Example:
  conductor --port 8080
  conductor --api-key sk-ant-xxx --repo /path/to/repo
