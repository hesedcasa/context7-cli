...
Conductor is a web-based orchestration platform for managing multiple Claude AI agents working in parallel on the same codebase. Inspired by [Conductor.build](https://conductor.build/), this implementation runs locally on Node.js and uses the [Claude Agent SDK](https://docs.claude.com/en/docs/agent-sdk/typescript) for TypeScript.
...
3. **Conductor Server** (`src/conductor/server.ts`)
...
The package is already installed with the context7-cli project. If you need to install it separately:
...
conductor --repo /path/to/your/repo
```
...
- A unique ID
- An isolated git worktree at `.conductor-worktrees/<agent-id>`
- A dedicated branch `conductor/<agent-name>-<short-id>`
...
Conductor uses [git worktrees](https://git-scm.com/docs/git-worktree) to provide isolated workspaces for each agent. This means:

- Each agent works in a separate directory (`.conductor-worktrees/<agent-id>`)
- Each agent has its own branch (`conductor/<name>-<id>`)
...
src/conductor-cli.ts      # CLI entry point
```
...
Ensure you're running Conductor from inside a git repository:
```bash
git init  # If needed
conductor
```
...
- Inspired by [Conductor.build](https://conductor.build/)
